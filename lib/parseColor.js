...
function rgb2hexStr(rgb) {
  function zp(h) { return (h < 16 ? '0' : '') + h.toString(16); }
  return '#' + zp(rgb.r) + zp(rgb.g) + zp(rgb.b);
}
...
function rgba2rgbaStr(rgba) {
  return 'rgba(' + Math.floor(rgba.r) + ',' + Math.floor(rgba.g) + ',' + Math.floor(rgba.b) + ',' + formatDecimal(rgba.a) + ')';
}
